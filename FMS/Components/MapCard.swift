...
public struct MapCard: View {
    ...
    private func fetchRoutes() async {
        ...
        for i in 0..<(stops.count - 1) {
            // iOS 26: MKMapItem(location:address:) replaces deprecated MKPlacemark + init(placemark:)
            let sourceItem = MKMapItem(
                location: CLLocation(latitude: stops[i].coordinate.latitude,
                                     longitude: stops[i].coordinate.longitude),
                address: nil
            )
            let destinationItem = MKMapItem(
                location: CLLocation(latitude: stops[i + 1].coordinate.latitude,
